...
impl ManagementBackend for WebRemoteBackend {
    ...
    fn search_mcp_market(
        ...
        let this = self.clone();
        Box::pin(async move {
            let payload = RemoteProviderRequest::McpMarketSearch(
                vibex_core::RemoteProviderMcpMarketSearchRequest {
                    auth: this.auth(),
                    request,
                },
            );
            // A catalog search reaches several remote sources, so it gets the
            // long-running class rather than the standard round-trip budget.
            let value = this
                .rpc(
                    RemoteOperationKind::ProviderSettings,
                    payload,
    ...
    fn install_mcp_market_entry(
        &self,
        request: MutationRequest<vibex_core::McpMarketInstallRequest>,
    ) -> BackendFuture<'_, vibex_core::McpMarketInstallResult> {
        let this = self.clone();
        Box::pin(async move {
            request.validate()?;
            let key = Self::mutation_key(&request);
            let payload = RemoteProviderRequest::McpMarketInstall(
                vibex_core::RemoteProviderMcpMarketInstallRequest {
                    auth: this.auth(),
                    request: request.payload,
                },
            );
            let value = this
                ...
                .await?;
            Ok(decode::<vibex_core::RemoteProviderMcpMarketInstallResponse>(value)?.result)
        })
    }
    ...
    fn search_skill_market(
        ...
        let this = self.clone();
        Box::pin(async move {
            let payload = RemoteProviderRequest::SkillMarketSearch(
                vibex_core::RemoteProviderSkillMarketSearchRequest {
                    auth: this.auth(),
                    request,
                },
            );
            let value = this
                ...
                .await?;
            Ok(decode::<vibex_core::RemoteProviderSkillMarketSearchResponse>(value)?.result)
        })
    }
    ...
    fn skill_market_document(
        ...
        let this = self.clone();
        Box::pin(async move {
            let payload = RemoteProviderRequest::SkillMarketDocument(
                vibex_core::RemoteProviderSkillMarketDocumentRequest {
                    auth: this.auth(),
                    request,
                },
            );
            let value = this
                ...
                .await?;
            Ok(decode::<vibex_core::RemoteProviderSkillMarketDocumentResponse>(value)?.document)
        })
    }

    fn install_skill_market_entry(
        &self,
        request: MutationRequest<vibex_core::SkillMarketInstallRequest>,
    ) -> BackendFuture<'_, vibex_core::SkillMarketInstallResult> {
        let this = self.clone();
        Box::pin(async move {
            request.validate()?;
            let key = Self::mutation_key(&request);
            let payload = RemoteProviderRequest::SkillMarketInstall(
                vibex_core::RemoteProviderSkillMarketInstallRequest {
                    auth: this.auth(),
                    request: request.payload,
                },
            );
            let value = this
                ...
                .await?;
            Ok(decode::<vibex_core::RemoteProviderSkillMarketInstallResponse>(value)?.result)
        })
    }
