...
struct AgentAuthPendingOperation {
    ...
    auth_context: Option<(vibex_core::AgentAuthContextId, i64)>,
}
...
enum AgentAuthTerminalCompletion {
    ...
    AgentAccount(AgentAuthContextMutationResult),
...
pub struct ManagementCenter {
    ...
    /// The Model the settings pane is showing, by id.
    ///
    /// It is an id rather than an index because the picker also lists Models
    /// that are not configured yet, and because editing a Model's id must not
    /// move the selection to whatever now sits at that position.
    profile_model_selection: Option<String>,
...
impl ManagementCenter {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let custom_agent_id = cx.new(|cx| InputState::new(window, cx).placeholder("my-acp-agent"));
        let custom_agent_label = cx.new(|cx| {
    ...
    pub fn show_agent_authentication(&mut self, agent_id: AgentId, cx: &mut Context<Self>) {
        self.navigation.active = ManagementSection::Agents;
        ...
        if self.selected_agent_id.as_deref() == Some(agent_id.as_str()) {
            ...
            self.load_agent_auth(true, cx);
        } else {
            self.select_management_agent(agent_id, cx);
        }
        cx.notify();
    }
    ...
    fn ensure_agent_auth_inputs(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.selected_provider_profile_id.is_none() {
            self.agent_auth_inputs.clear();
            self.agent_auth_clear_values.clear();
            return;
        }
    ...
    fn authenticate_agent_context(
        ...
        if method.effect == AgentAuthMethodEffect::RequiresProviderProfile {
            self.agent_auth_error = Some(
                management_error_text(
                    "This sign-in method requires a model provider configuration",
                    "此登录方式需要模型供应商配置",
                    "此登入方式需要模型供應商設定",
                )
                .to_string(),
        ...
        let agent_id_key = context.agent_id.as_str().to_string();
        if self.mutation.is_some() || self.agent_mutations.contains_key(&agent_id_key) {
            return;
        }
        let generation = self.agent_auth_generation;
        ...
        self.agent_auth_operations.insert(
            ...
            AgentAuthPendingOperation {
                ...
                auth_context: Some((context.id.clone(), context.revision)),
            },
        );
        self.agent_auth_logout_preview = None;
        self.agent_auth_error = None;
        ...
        let task = cx.spawn(async move |_, cx| {
            ...
            let _ = entity.update(cx, |this, cx| {
                ...
                let interactive = matches!(
                    &outcome,
                    Ok(Ok(result)) if result.terminal.is_some()
                );
                if operation_is_registered && !(interactive && scope_is_current) {
                    ...
                    Ok(Ok(result)) => {
                        ...
                        if let Some(terminal) = result.terminal {
                            ...
                            this.notice = Some(
                                management_locale_text_for(
                                    active_locale,
                                    "Default Agent account verified",
                                    "Agent 默认账号已验证",
                                    "Agent 預設帳號已驗證",
                                )
                                .to_string(),
    ...
    fn authenticate_agent(&mut self, method_id: String, cx: &mut Context<Self>) {
        ...
        if scope.1.is_none() {
            self.authenticate_agent_context(method_id, scope, cx);
            return;
        }
        ...
        let task = cx.spawn(async move |_, cx| {
            ...
            let _ = entity.update(cx, |this, cx| {
                ...
                let interactive = matches!(
                    &outcome,
                    Ok(Ok((_, Some(terminal), _))) if terminal.terminal_id.is_some()
                );
                if operation_is_registered && !(interactive && scope_is_current) {
                    this.agent_auth_operations.remove(&agent_id_for_callback);
                }
    ...
    fn cancel_agent_authentication(
        ...
        operation.phase = AgentAuthOperationPhase::Cancelling;
        let auth_context = operation.auth_context.clone();
        self.agent_auth_error = None;
                ...
                match outcome {
                    ...
                    Ok(Ok(false)) => {
                        if let Some(operation) =
                            this.agent_auth_operations.get_mut(&agent_id_for_callback)
                        {
    ...
    fn refresh_agent_auth_models(&mut self, cx: &mut Context<Self>) {
        ...
        let mutation = ManagementMutation::AgentAuth {
            agent_id: agent_id_key.clone(),
            action: "refresh-models".to_string(),
        };
        self.agent_mutations
            .insert(agent_id_key.clone(), mutation.clone());
        self.agent_auth_error = None;
                ...
                match outcome {
                    Ok(Ok(result)) => {
                        ...
                        this.notice = Some(format!(
                            "{} ({model_count})",
                            management_locale_text(
                                "Agent account models refreshed",
                                "Agent 账号模型已刷新",
                                "Agent 帳號模型已重新整理",
                            )
                        ));
    ...
    fn preview_agent_auth_context_logout(
        &mut self,
        scope: (String, Option<String>),
        cx: &mut Context<Self>,
    ) {
        ...
        let mutation = ManagementMutation::AgentAuth {
            agent_id: agent_id_key.clone(),
            action: "logout-preview".to_string(),
        };
        self.agent_mutations
            .insert(agent_id_key.clone(), mutation.clone());
        self.agent_auth_error = None;
        ...
        let task = cx.spawn(async move |_, cx| {
            ...
            let _ = entity.update(cx, |this, cx| {
                ...
                match outcome {
                    Ok(Ok(preview)) => {
                        this.agent_auth_logout_preview = Some(preview);
                        this.agent_auth_error = None;
                    }
                    Ok(Err(error)) => {
                        this.agent_auth_error = Some(format!("{}: {}", error.code, error.message));
    ...
    fn cancel_agent_auth_context_logout(&mut self, cx: &mut Context<Self>) {
        self.agent_auth_logout_preview = None;
        cx.notify();
    }
    ...
    fn confirm_agent_auth_context_logout(&mut self, cx: &mut Context<Self>) {
        ...
        let agent_id_key = preview.context.agent_id.as_str().to_string();
        if self.mutation.is_some() || self.agent_mutations.contains_key(&agent_id_key) {
            return;
        }
        let generation = self.agent_auth_generation;
        ...
        let task = cx.spawn(async move |_, cx| {
            ...
            let _ = entity.update(cx, |this, cx| {
                ...
                match outcome {
                    Ok(Ok(result)) => {
                        let affected_count = result.affected_session_ids.len();
                        this.agent_auth_context = Some(result.context);
                        ...
                        this.notice = Some(format!(
                            ...
                            management_locale_text_for(
                                active_locale,
                                "Default Agent account signed out; affected sessions",
                                "Agent 默认账号已退出；受影响会话数",
                                "Agent 預設帳號已登出；受影響工作階段數",
                            )
                        ));
                    }
                    Ok(Err(error)) => {
    ...
    fn close_agent_auth_terminal(&mut self, cx: &mut Context<Self>) {
        if self.agent_auth_terminal_state == Some(AgentAuthTerminalState::Running)
            && let Some(agent_id) = self.selected_agent_id.clone()
        ...
        self.agent_auth_terminal_monitor_task = Some(cx.spawn(async move |_, cx| {
            ...
            let _ = entity.update(cx, |this, cx| {
                ...
                if this
                    ...
                    .is_some_and(|operation| operation.operation_id == operation_id)
                {
                    this.agent_auth_operations.remove(&scope.0);
                }
                match outcome {
                    Ok(Ok(AgentAuthTerminalCompletion::AgentAccount(result))) => {
                        ...
                        this.notice = Some(
                            management_locale_text_for(
                                active_locale,
                                "Interactive Agent sign-in completed and verified",
                                "Agent 交互式登录已完成并通过验证",
                                "Agent 互動式登入已完成並通過驗證",
                            )
                            .to_string(),
                    ...
                    Ok(Ok(AgentAuthTerminalCompletion::Authenticated { .. })) => {
                        this.agent_auth_terminal_state = Some(AgentAuthTerminalState::Failed);
                        this.agent_auth_error = Some(
                            "agent_auth_terminal_completion_mismatch: Unexpected authentication result"
                                .to_string(),
                        );
                    }
                    Err(error) => {
    ...
    fn add_profile_model(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.choose_profile_model(model_id.clone(), None);
        self.profile_model_draft
            .update(cx, |state, cx| state.set_value("", window, cx));
        // A hand-typed Model has no row to click, so adding it is also the
        // gesture that opens its settings.
        self.select_profile_model(model_id, window, cx);
        self.navigation.mark_dirty(ManagementSection::Agents, true);
        cx.notify();
    }

    /// Adds the Model the settings pane is showing and opens its editor.
    fn add_selected_profile_model(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let Some(model_id) = self.profile_model_selection.clone() else {
            return;
        };
        let entry = self
            ...
            .map(|row| row.as_configured_model());
        if self.choose_profile_model(model_id.clone(), entry) {
            self.navigation.mark_dirty(ManagementSection::Agents, true);
        }
        self.select_profile_model(model_id, window, cx);
    }
    ...
    /// The configured Model the settings pane is editing, if the selection
    /// still names one.
    fn selected_profile_model_index(&self) -> Option<usize> {
        let selected = self.profile_model_selection.as_deref()?;
    ...
    fn select_profile_model(
        ...
        self.profile_model_selection = Some(model_id.clone());
        match self
            .profile_configured_models
            .iter()
            .position(|model| model.id == model_id)
    ...
    /// Loads one configured Model into the settings pane's fields.
    fn load_profile_model_editor(
        &mut self,
        index: usize,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        self.profile_model_selection = Some(model.id);
        self.profile_model_edit_error = None;
    ...
    fn apply_profile_model_edit(&mut self, cx: &mut Context<Self>) {
        let Some(index) = self.selected_profile_model_index() else {
            return;
        };
        ...
        let renamed = if model_id.is_empty() {
            None
        } else {
        ...
        // The selection names the Model by id, so a rename has to carry it or
        // the pane would lose the Model it is editing.
        if let Some(renamed) = renamed {
            self.profile_model_selection = Some(renamed);
        }
    ...
    fn create_custom_agent(&mut self, cx: &mut Context<Self>) {
        ...
        let Ok(agent_id) = AgentId::parse(id) else {
            self.error = Some(
                management_locale_text(
                    ...
                    "Agent ID 必须使用小写短横线格式",
                    "Agent ID 必須使用小寫連字號格式",
                )
                .into(),
            );
            cx.notify();
        ...
        if label.is_empty() || command.is_empty() {
            self.error = Some(
                management_locale_text(
                    "Display name and command are required",
                    "显示名称和命令不能为空",
                    "顯示名稱和命令不能為空",
                )
                .into(),
        ...
        self.custom_agent_editor_open = false;
        self.selected_agent_id = Some(selected_id.clone());
        self.begin_simple_task(
    ...
    fn set_agent_added(&mut self, agent_id: String, added: bool, cx: &mut Context<Self>) {
        ...
        let managed = selected_agent.is_some_and(|agent| agent.managed_install.managed);
        let custom = selected_agent
            .is_some_and(|agent| agent.source_kind == vibex_core::AgentSourceKind::Custom);
        if managed && added {
            ...
            async move {
                ...
                if custom && !added {
                    ...
                    return Ok(management_locale_text_for(
                        ...
                        "自定义 Agent 已删除，会话历史已保留",
                        "自訂 Agent 已刪除，工作階段歷史已保留",
                    )
                    .to_string());
                }
                providers
    ...
    /// The picker: every Model the endpoint advertised, plus the ones the draft
    /// already configures, each with the checkbox that decides whether this
    /// Provider offers it.
    fn render_profile_candidates_pane(
        ...
        let visible = rows
            ...
            .collect::<Vec<_>>();
        let configured = self.profile_configured_models.len();
        ...
        // row once the list is long enough to scan.
    ...
    fn render_profile_candidate_row(
        ...
        let opened = self.profile_model_selection.as_deref() == Some(row.id.as_str());
    ...
    /// The settings pane: the editable fields of the Model the picker has
    /// selected, or the command that adds it when the Provider does not offer
    /// it yet.
    fn render_profile_settings_pane(
        &mut self,
        bounded: bool,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        ...
        let selection = self.profile_model_selection.clone();
        let selected_index = self.selected_profile_model_index();
        let selected_title = selection.as_deref().map(|id| {
            self.profile_configured_models
                .iter()
                ...
                Some(title) => v_flex()
                    ...
                    .child(
                        div()
                            ...
                            .text_color(cx.theme().muted_foreground)
                            .child(management_locale_text(
                                "This Provider does not offer this model yet.",
                                "该供应商尚未提供这个模型。",
                                "該供應商尚未提供這個模型。",
                            )),
                    )
                    .child(
                        Button::new("provider-model-add-selected")
                            .small()
                            .secondary()
                            .icon(IconName::Plus)
                            .label(management_locale_text(
                            ...
                            .disabled(pending)
                            .on_click(cx.listener(|this, _, window, cx| {
                                this.add_selected_profile_model(window, cx)
                            })),
                    )
                    .into_any_element(),
                        ...
                        v_flex()
                            ...
                            .child(
                                div()
                                    ...
                                    .text_color(cx.theme().muted_foreground)
                                    .child(management_locale_text(
                                        "A disabled model keeps its settings but is not offered to the Agent.",
                                        "停用后仍保留配置，但不会提供给 Agent。",
                                        "停用後仍保留設定，但不會提供給 Agent。",
                                    )),
                            ),
                    )
                    .child(
    ...
    fn set_visible_profile_models_enabled(&mut self, enabled: bool, cx: &mut Context<Self>) {
        // The picker is the only list, so "visible" means the rows it is
        // showing that the Provider already offers.
        let filter = self
            .profile_model_search
            .read(cx)
            .value()
        ...
        for model in self.profile_configured_models.iter_mut() {
            if ids.contains(&model.id) && model.enabled != enabled {
                model.enabled = enabled;
                changed = true;
            }
        }
        ...
        if wide {
            ...
            if shows_model {
                stacked = stacked
                    ...
                    .child(self.render_profile_settings_pane(false, cx));
            }
            body = body.child(
                div()
                    .min_h_0()
    ...
    fn render_agent_authentication(
            ...
            .is_some_and(|operation| operation.phase == AgentAuthOperationPhase::Cancelling)
        {
            ...
            match context.as_ref().map(|context| context.status) {
                ...
                Some(AgentAuthContextStatus::AuthenticationRequired) => {
                    management_locale_text("Sign-in required", "需要登录", "需要登入")
                }
                Some(AgentAuthContextStatus::Verifying) => {
                    management_locale_text("Verifying", "正在验证", "正在驗證")
                }
                Some(AgentAuthContextStatus::Unavailable) => {
                    management_locale_text("Temporarily unavailable", "暂不可用", "暫不可用")
                }
                Some(AgentAuthContextStatus::Unverified) => {
                    management_locale_text("Not verified", "尚未验证", "尚未驗證")
                }
                None if self.agent_auth_loading => {
        ...
        let logout_loading = selected_agent_id.as_deref().is_some_and(|agent_id| {
            matches!(
                ...
                Some(ManagementMutation::AgentAuth { action, .. })
                    if action == "logout" || action == "logout-preview"
            )
        });
        let verify_loading = selected_agent_id.as_deref().is_some_and(|agent_id| {
            matches!(
                self.agent_mutations.get(agent_id),
                Some(ManagementMutation::AgentAuth { action, .. }) if action == "verify"
            )
        });
        let models_loading = selected_agent_id.as_deref().is_some_and(|agent_id| {
            matches!(
                self.agent_mutations.get(agent_id),
                Some(ManagementMutation::AgentAuth { action, .. }) if action == "refresh-models"
            )
        });
                ...
                .child(
                    v_flex()
                        ...
                        .when_some(
                            context
                                .as_ref()
                                .and_then(|context| context.account_hint.clone()),
                            |header, hint| {
                                header.child(
                                    div()
                                        .truncate()
                        ...
                        .when(context.is_some() && !native_authenticated, |actions| {
                            actions.child(management_detail_icon_action(
                                Button::new("agent-auth-verify")
                                    ...
                                    .disabled(pending)
                                    .on_click(cx.listener(|this, _, _, cx| {
                                        this.verify_agent_auth_context(cx)
                                    })),
            ...
            content = content.child(
                v_flex()
                    ...
                    .child(div().text_sm().font_medium().child(format!(
                        "{}: {affected_count}",
                        management_locale_text(
                            "Signing out also affects the external CLI and Vibex sessions",
                            "退出也会影响外部 CLI 与 Vibex 会话",
                            "登出也會影響外部 CLI 與 Vibex 工作階段",
                        )
                    )))
                    .child(
                        h_flex()
                            ...
                            .gap_2()
                            .child(
                                Button::new("agent-auth-logout-cancel")
                                    .small()
                                    .outline()
                                    .label(management_locale_text("Cancel", "取消", "取消"))
                                    .disabled(logout_loading)
                                    .on_click(cx.listener(|this, _, _, cx| {
                                        this.cancel_agent_auth_context_logout(cx)
                                    })),
                            )
                            .child(
                                Button::new("agent-auth-logout-confirm")
                                    .small()
                                    .danger()
                                    .label(management_locale_text(
                                        "Confirm sign out",
                                    ...
                                    .disabled(logout_loading)
                                    .on_click(cx.listener(|this, _, _, cx| {
                                        this.confirm_agent_auth_context_logout(cx)
                                    })),
                            ),
                    ),
            );
        ...
        if let Some(catalog) = catalog {
            ...
            if methods.is_empty() {
                content = content.child(compact_empty_state(
                    management_locale_text(
                        "No default-account sign-in method reported",
                        "Agent 未上报默认账号登录方式",
                        "Agent 未回報預設帳號登入方式",
                    ),
                    management_locale_text(
                        "Use Verify account to reuse an existing external CLI sign-in.",
                        "可验证并复用外部 CLI 中已有的登录状态。",
                        "可驗證並重用外部 CLI 中既有的登入狀態。",
                    ),
                    cx,
                ));
            }
...
mod tests {
    ...
    fn provider_editor_pairs_a_model_picker_with_its_settings() {
        ...
        let settings = source
            .split_once("    fn render_profile_settings_pane(")
        ...
        // The catalogue is a checked list, and the custom id field adds to it.
    ...
    #[gpui::test]
    fn selecting_a_picker_row_loads_that_models_settings(cx: &mut gpui::TestAppContext) {
        cx.update(gpui_component::init);
        let (center, cx) = cx.add_window_view(ManagementCenter::new);
        cx.update(|window, cx| {
            center.update(cx, |center, cx| {
                ...
                // A configured Model opens its own settings.
                center.select_profile_model("gpt-5".to_string(), window, cx);
                assert_eq!(center.selected_profile_model_index(), Some(0));
                assert_eq!(center.profile_model_edit_id.read(cx).value(), "gpt-5");
                assert_eq!(
                    center.profile_model_edit_context_tokens.read(cx).value(),
                    "128000"
                );

                // One the Provider does not offer yet has no settings to load.
                center.select_profile_model("gpt-5-mini".to_string(), window, cx);
                assert_eq!(center.selected_profile_model_index(), None);
                assert_eq!(
                    center.profile_model_selection.as_deref(),
                    Some("gpt-5-mini")
                );

                // Adding it from the settings pane opens its editor.
                center.add_selected_profile_model(window, cx);
                assert_eq!(center.profile_configured_models.len(), 2);
                assert_eq!(center.selected_profile_model_index(), Some(1));
            });
        });
    }
    ...
    fn provider_editor_draws_its_model_panes(cx: &mut gpui::TestAppContext) {
        ...
        center.update(cx, |center, _| {
            ...
            chosen.capabilities.context_tokens = Some(128_000);
            center.profile_configured_models = vec![chosen];
            // The settings pane always shows one Model, which is the tallest
            // the right column ever gets.
    ...
    fn agent_detail_modules_use_accessible_icon_actions() {
        ...
        assert_eq!(
            authentication.matches(".label(").count(),
            2,
            "only the destructive sign-out confirmation may use text buttons"
        );
    ...
    #[test]
    fn default_agent_account_uses_context_state_and_a_confirmed_global_lifecycle() {
        let source = include_str!("management.rs");
        let render = source
            .split_once("    fn render_agent_authentication(")
            .and_then(|(_, tail)| tail.split_once("\n    fn render_agent_install_loading("))
        ...
        let lifecycle = source
            .split_once("    fn verify_agent_auth_context(")
            .and_then(|(_, tail)| tail.split_once("\n    fn clear_agent_auth_terminal("))
            .map(|(body, _)| body)
        ...
        assert!(render.contains("agent-auth-refresh-models"));
