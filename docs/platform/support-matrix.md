...
| Web/PWA | GPUI-WASM remote client | `pnpm check:wasm-web` | Requires WebGPU. Browser accessibility and high-DPI backing-store gaps block a production support claim. |
| Android | Capacitor shell for the shared GPUI-WASM client | `pnpm --filter @vibex/mobile android:debug` | APK construction is supported; each rebuilt APK needs source-bound physical-device validation before a release claim. |
| iOS | Capacitor shell for the shared GPUI-WASM client | `pnpm --filter @vibex/mobile ios:debug` on macOS | Requires Xcode and physical/simulator validation. No iOS production support claim is currently published. |
...
Desktop is the only authoritative runtime. Web and mobile are remote clients
through `WebRemoteBackend`; they do not run local Agents, Git, PTY, or workspace
filesystem services. The shared UI and Wide/Medium/Compact shell rules live in
`crates/vibex-ui`.

## Capability Boundaries

| Capability | Native desktop | Web/mobile |
| --- | --- | --- |
...
- GPUI-WASM requires WebGPU and currently lacks a complete browser accessibility
  tree.
- Browser/WebView IME, clipboard, keyboard layout, soft keyboard, lifecycle,
